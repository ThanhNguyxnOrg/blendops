...
> [!NOTE]
> BlendOps publishes one thing: a **skills layer** (24 skills (Batch 1 + 2 of 8 added 2026-05-09; roadmap to 48 skills = 3x) + 4 laws + 1 pack + 3 bundles + 16 install target docs + ZIP/package generation + CI guards). The layer is **complete and shippable** at Draft v0.
>
...
**Bottom line**: clone the repo today → paste 30-second prompt → agent auto-detects and installs/prepares ZIPs → plan a scene with the 24 skills → follow per-target install doc to enable an upstream runtime path → drive Blender via Path 1 / Path 2 (both verified by the repo owner) or CLI (decades-stable). Skills layer is **shipped as `v0.1.0-draft` (16 skills baseline) with Batch 1 + 2 (8 new skills total) added on top**; runtime paths are **upstream's deliverable**, not BlendOps'. Roadmap: 48 skills total (3x baseline) across 8 batches; tag `v0.2.0-draft` after Batch 2 closes (24 skills milestone).
