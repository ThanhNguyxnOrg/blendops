...
| Runtime bridge layer | One of the runtime paths (see [`./runtime-stack-strategy.md`](./runtime-stack-strategy.md)): Path 1 (Official Blender Lab MCP, hosted from either (a) Anthropic Blender Connector or (b) any other MCP client configured manually) or Path 2 (community `ahujasid/blender-mcp`). CLI fallback exists as appendix only — documented upstream as first-class Blender CLI surface, no in-repo evidence file yet. | Separate setup owned by upstream docs and the local user environment; not installed by BlendOps. |
...
| Claude Code | Project-local files or verified tool-native skill path. | Path 1 host (a) Anthropic Connector is Claude Desktop only and not applicable; Path 1 host (b) manual MCP usable; Path 2 usable; CLI fallback usable; documented upstream as first-class Blender CLI but no in-repo evidence file yet. | Project-local file install | Attach `skills/`, `laws/`, `packs/`, and entrypoint instructions. | verified-read for project-local adapter; native path environment-dependent | Keep Claude Code distinct from Claude Desktop. |
| OpenCode | Project-local files now; future tool-native install option if verified. | Path 1 host (a) not applicable; Path 1 host (b) usable; Path 2 usable; CLI fallback usable; documented upstream as first-class Blender CLI but no in-repo evidence file yet. | Project-local file install / future verified tool-native install | Add project-local entrypoint or verified config later. | linked-only | Do not claim plugin availability yet. |
| Cursor | Project-local rules/docs attachment if verified. | Path 1 host (a) not applicable; Path 1 host (b) usable; Path 2 usable; CLI fallback usable; documented upstream as first-class Blender CLI but no in-repo evidence file yet. | Project-local file install | Add project rules or docs references only after backup/approval. | linked-only | Avoid broad global rule mutation. |
| Codex CLI/App | Generic project instruction or future adapter. | Path 1 host (a) not applicable; Path 1 host (b) usable; Path 2 usable; CLI fallback usable; documented upstream as first-class Blender CLI but no in-repo evidence file yet. | Project-local file install / Docs-only/reference | Use AGENTS-style fallback when native path is unknown. | linked-only | Do not invent plugin paths. |
| Gemini CLI | Future adapter or docs-only reference until researched. | Path 1 host (a) not applicable; MCP support TBD per upstream; CLI fallback usable; documented upstream as first-class Blender CLI but no in-repo evidence file yet. | Docs-only/reference | Treat as not researched until verified. | not researched | Add adapter only after source-backed path exists. |
