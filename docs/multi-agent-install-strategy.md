...
| Runtime bridge layer | One of the four runtime routes (see [`./runtime-stack-strategy.md`](./runtime-stack-strategy.md)): Route A (Anthropic Blender Connector), Route B (Blender Foundation MCP `bpype/blender_mcp`), Route C (community `ahujasid/blender-mcp`), or Route D (Blender CLI). | Separate setup owned by upstream docs and the local user environment; not installed by BlendOps. |
...
| Claude Code | Project-local files or verified tool-native skill path. | Route A is Claude Desktop only and not applicable; Routes B/C usable via MCP config; Route D usable via shell. | Project-local file install | Attach `skills/`, `laws/`, `packs/`, and entrypoint instructions. | verified-read for project-local adapter; native path environment-dependent | Keep Claude Code distinct from Claude Desktop. |
| OpenCode | Project-local files now; future tool-native install option if verified. | Route A not applicable; Routes B/C usable via MCP config; Route D usable via shell. | Project-local file install / future verified tool-native install | Add project-local entrypoint or verified config later. | linked-only | Do not claim plugin availability yet. |
| Cursor | Project-local rules/docs attachment if verified. | Route A not applicable; Routes B/C usable via MCP config; Route D usable via shell. | Project-local file install | Add project rules or docs references only after backup/approval. | linked-only | Avoid broad global rule mutation. |
| Codex CLI/App | Generic project instruction or future adapter. | Route A not applicable; Routes B/C usable via MCP config; Route D usable via shell. | Project-local file install / Docs-only/reference | Use AGENTS-style fallback when native path is unknown. | linked-only | Do not invent plugin paths. |
| Gemini CLI | Future adapter or docs-only reference until researched. | Route A not applicable; MCP support TBD per upstream; Route D usable via shell. | Docs-only/reference | Treat as not researched until verified. | not researched | Add adapter only after source-backed path exists. |
| Generic root/project-local fallback | `BLENDOPS.md`, `AGENTS.md`, or copied core collection references. | Route A not applicable; Routes B/C usable via MCP config; Route D usable via shell. | Project-local file install | Copy/reference core collection and record rollback. | verified-read | Default safest reversible path. |
