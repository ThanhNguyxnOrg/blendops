...
| [blender-connector-read-only-smoke-test.md](./blender-connector-read-only-smoke-test.md) | Pass / Available (Path 1 read-only, likely Anthropic Connector host) | Read-only smoke evidence | Lab MCP tool surface (`get_blendfile_summary_*`, `get_objects_summary`) returned `status: ok`. No mutation, render, export, or artifacts. |
| [path-2-ahujasid-readonly-template.md](./path-2-ahujasid-readonly-template.md) | Template only / Not Yet Run | Fill-in template | Use this template to record Path 2 read-only smoke evidence when running `ahujasid/blender-mcp`. |
