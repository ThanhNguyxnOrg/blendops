...
1. **Preferred:** upload/import `bundles/skill-package/blendops/` as one Skill package if your Claude Desktop workflow supports package upload/import. The `agents/openai.yaml` file in that package is OpenAI/ChatGPT Skills UI metadata only and does not configure Claude Desktop Connector.
